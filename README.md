...
| 기능 | 설명 |
|------|------|
| **사내 문서 검색 (RAG)** | Chroma 벡터 DB + Gemini 임베딩으로 사내 지식 베이스 검색 · 출처 페이지 번호 표시 · 품질 부족 시 쿼리 재작성 후 재검색(최대 2회) |
...
| 카테고리 | 처리 방식 |
|---|---|
| `knowledge_search` | ChromaDB 검색 → 품질 체크 → (필요 시 쿼리 재작성 후 재검색) → LLM 답변 |
...
│   │       ├── knowledge_search.py # search / quality_check / rewrite / answer 4노드
