...
| **시맨틱 라우팅** | 코사인 유사도 + LLM fallback + 슬롯 기반 clarification · 무한 루프 방지 (count≥2) · RFP/Email/File 슬롯 감지 + 구조화 |
...
```
복합 요청 감지 (task_router)
    └─ 선행 키워드(검색/조회/찾아/확인해) + 후행 키워드(rfp/이메일/초안/작성해) 동시 존재
         → task_type = "planner"
...
## 라우팅 시스템 개선 (Phase 1~4 + Priority 3)

### 개선 목표
"분류 정확도 우선" → **"실행 우선, clarification 최소화"**

### 핵심 개선사항

#### Phase 1: Unknown Task 기본값 변경
```
기존: unknown → 무조건 clarification
개선: unknown → ai_guide (짧은 질문) 또는 knowledge_search (일반 질문)
효과: "뭐 할 수 있어?" 같은 모호한 입력 → 즉시 응답
```

#### Phase 2: LLM Fallback 신뢰도 완화
```
기존: 0.7 미만 → unknown
개선: 0.5 미만 → unknown / 0.5~0.7 → 실행(low_confidence 플래그) / 0.7 이상 → 정상
효과: LLM도 합리적인 분류는 실행, clarification 발동 감소
```

#### Phase 3-A: State 루프 방어 필드
```
추가: clarification_count: int (발동 횟수 추적)
```

#### Phase 3-B: Clarification 루프 방어 + 슬롯 기반 질문
```
루프 방어: clarification_count ≥ 2 → 강제 knowledge_search fallback
슬롯 질문: missing_slots 있으면 LLM 우회, 고정 질문 사용
       • "file_path" → "분석할 파일 경로를 알려주세요"
       • "to" → "이메일 수신자를 알려주세요. (예: hr@example.com 또는 인사팀)"
       • "project_scope" → "RFP 범위를 알려주세요"
슬롯 구조화: user_response → task_args[slot명] 저장 → 다음 라우팅에서 재사용
task_args 보존: LLM clarification 재개 시 context 유지
```

#### Phase 4: Email Draft 추가 검증 완화
```
기존: 힌트 없으면 무조건 unknown
개선: 힌트 없고 semantic score ≤ 0.70일 때만 unknown
효과: 신뢰도 높은 이메일 요청은 힌트 없어도 실행
```

#### Priority 3: 슬롯 감지 + 구조화
```
신규 감지 함수:
  • _has_recipient_hint() — 이메일 주소 or 부서명 포함
  • _has_rfp_scope_hint() — 한국어 키워드(제안, 설계, 구현 등) + 영어 정규식 패턴

슬롯 검증 로직:
  • file_extract: file_path 감지 없으면 & task_args["file_path"] 없으면 → clarification
  • email_draft: recipient 감지 없으면 & task_args["to"] 없으면 → clarification
  • rfp_draft: scope 감지 없으면 & task_args["project_scope"] 없으면 → clarification

효과:
  ✓ "이메일 작성해줘" → clarification → "hr팀" → email_draft 직행
  ✓ "RFP for a new ERP system proposal" → rfp_draft 즉시 실행
  ✓ 슬롯 응답이 구조화되어 다음 라우팅이 올바르게 인식
```
...
| 입력 | 기존 | 개선 |
|------|------|------|
| "뭐 할 수 있어?" | clarification | ai_guide ✅ |
| "이메일 작성해줘" | clarification | clarification → "hr팀" → email_draft ✅ |
| "RFP 새 ERP 제안" | clarification | rfp_draft (직행) ✅ |
| clarification 2회 반복 | 계속 반복 (무한 루프) | knowledge_search 강제 fallback ✅ |
...
│   │       ├── task_router.py     # 시맨틱 라우터 노드 + 복합 요청 감지 + 라우팅 함수
...
LLM_FALLBACK_CONFIDENCE_MIN=0.5           # LLM fallback 신뢰도 임계값 (기본 0.5)
EMAIL_DRAFT_HIGH_CONF_THRESHOLD=0.70      # email_draft semantic score 임계값 (기본 0.70)
