...
def build_chat_subgraph():
    ...
    def chat_agent_node(state: GraphState) -> Dict[str, Any]:
        ...
        system_content = (
            ...
            "  이 경우 이전 대화에서 검색된 사내 문서 내용이나 인물 정보를 답변에 포함하지 마세요.\n"
            "• 대화 히스토리 자체를 보여달라는 요청(예: '이전 대화 알려줘', '히스토리 요약해줘') →\n"
            "  '대화 내역은 별도로 제공하지 않습니다. 업무 관련 질문을 입력해 주세요.'라고만 안내하세요.\n"
            "• 사용자가 이전 대화를 명시적으로 참조할 때(예: '아까 그 문서', '방금 검색한 내용', '이전에 말한')에만\n"
            "  히스토리를 맥락으로 활용하세요. 그 외에는 현재 질문만 기준으로 독립적으로 답변하세요.\n"
            ...
            "【범위 외 질문】\n"
            "날씨·주식·스포츠·개인 일정 등 사내 업무와 무관한 질문:\n"
